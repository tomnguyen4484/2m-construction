...
const SCOPES: Record<Scope, { label: string; desc: string; perSqft: number; icon: string }> = {
  cosmetic:  { label: 'Cosmetic Update',  icon: '🖌️', perSqft: 70,  desc: 'Cabinet reface, backsplash, hardware, paint — keep layout' },
  standard:  { label: 'Standard Remodel', icon: '🍳', perSqft: 180, desc: 'New cabinets, countertops, sink, appliances — same layout' },
  full:      { label: 'Full Gut Remodel', icon: '🏗️', perSqft: 320, desc: 'Complete teardown — custom cabinets, layout change, everything new' },
};
  ...
  async function submitQuote() {
    ...
    try {
      await fetch('/api/contact', {
        ...
        body: JSON.stringify({ name: form.name, phone: form.phone, email: form.email, service: 'Kitchen Remodel', estimate: fmt(total), note: form.note }),
      });
      setSent(true);
    } catch { setSent(true); }
  }
    ...
    <div style={{ maxWidth:'680px', margin:'0 auto', padding:'20px 16px 40px' }}>
      ...
      <div style={{ background:'#FEF3C7', border:'1px solid #F59E0B', borderRadius:'10px', padding:'12px 14px', marginBottom:'20px' }}>
        ...
        <p style={{ fontSize:'12px', color:'#92400E', margin:0, lineHeight:1.6 }}>
          Final cost depends heavily on <strong>kitchen size, cabinet style, countertop material, appliances, and custom work</strong>.
      ...
      <div style={{ marginBottom:'28px' }}>
        ...
        <p style={{ fontSize:'11px', color:'#94A3B8', margin:'4px 0 0' }}>
          Small kitchen: under 100 sq ft · Average: 150–200 sq ft · Large: 250+ sq ft
        </p>
      </div>
        ...
        <div style={{ background:'linear-gradient(135deg,#1A3A5C,#0F2542)', borderRadius:'16px', padding:'24px', marginBottom:'20px', color:'#fff' }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.08)', borderRadius:'8px', padding:'10px 12px', marginTop:'12px' }}>
            <p style={{ fontSize:'11px', color:'rgba(255,255,255,0.55)', margin:0, lineHeight:1.6 }}>
              ⚠️ Price varies significantly based on cabinet style, countertop material, appliance brands, and custom work.
      ...
      {showContact && (
        <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:'16px', padding:'20px', marginBottom:'16px' }}>
          ...
          <textarea value={form.note} onChange={e => setForm({...form, note:e.target.value})} rows={3}
            ...
            placeholder="Describe your kitchen and what you'd like done..." />
