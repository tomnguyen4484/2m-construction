...
export default function FenceEstimator() {
  const [type, setType] = useState<'wood'|'vinyl'|'chain'>('wood');
  ...
  const material = ft * Number(height) * PRICE[type].material;
  const labor = ft * PRICE[type].labor;
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      <Link href="/estimate" className="text-sm text-gray-500 mb-4 block">← All Services</Link>
      <h1 className="text-xl font-bold text-gray-800 mb-1">🪵 Fence Estimator</h1>
      <p className="text-gray-500 text-sm mb-5">Prices updated from Home Depot</p>

      {/* Type */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-2">Fence Type</label>
        <div className="grid grid-cols-3 gap-2">
          {([['wood','🪵 Wood'],['vinyl','⬜ Vinyl'],['chain','⛓️ Chain Link']] as const).map(([v,l])=>(
      ...
      {/* Height */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-2">Height: {height} ft</label>
        <div className="grid grid-cols-3 gap-2">
          {['4','6','8'].map(h=>(
